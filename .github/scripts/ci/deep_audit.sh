FILES=$(find . -type d \( -name "target" -o -name "node_modules" -o -name "dist" -o -name "generated" -o -name "docs" -o -name "api" \) -prune -o -type f \( \( -name "*.ts" -o -name "*.rs" \) ! -name "*.sh" ! -name "*.config.ts" \) -print)
