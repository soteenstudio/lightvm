...
import * as instr from "./instr/index.js";
...
export let wasm: any = loadWasm("main.wasm");
let napi = loadNapi();
...
export function runBytecode(
  ...
  let ctx: { JITCF: boolean } = { JITCF: false };
  while (ip < bytecode.length) {
    ...
    switch (op) {
      ...
      case "concat": {
        ...
        // Gabungin dulu a dan b
        let combined = a + b;
        
        // Baru bersihin semuanya sekaligus
        const result = combined
          .replace(/['"]/g, "")      // Hapus semua petik (global)
          .replace(/::string/g, "")  // Hapus semua tag ::string
          .replace(/::space/g, " "); // Ganti semua ::space jadi spasi beneran
      ...
      case "add": {
        const b = pop(), a = pop();
        let result: number | string = napi.add(a as number, b as number, arg as NumType);
        //stack.push(typeof result === "string" ? result.replace(/::string/g, "") : result.replace("::space", " "));
        break;
      }
      case "sub": {
        const b = pop(), a = pop();
        if (arg === undefined) return 0;
        let result: number = napi.sub(a as number, b as number, arg as NumType);
        stack.push(result);
        break;
      }
      case "mul": {
        const b = pop(), a = pop();
        if (arg === undefined) return 0;
        let result: number = napi.mul(a as number, b as number, arg as NumType);
        stack.push(result);
        break;
      }
      case "div": {
        const b = pop(), a = pop();
        if (arg === undefined) return 0;
        let result: number = napi.div(a as number, b as number, arg as NumType);
        stack.push(result);
        break;
      }
      case "mod": {
        const b = pop(), a = pop();
        if (arg === undefined) return 0;
        let result: number = napi.modu(a as number, b as number, arg as NumType);
        stack.push(result);
        break;
      }
      case "gt": {
        const b = pop(), a = pop();
        if (arg === undefined) return 0;
        let result: boolean = napi.gt(a as number, b as number, arg as NumType);
        stack.push(result);
        break;
      }
      case "lt": {
        const b = pop(), a = pop();
        if (arg === undefined) return 0;
        let result: boolean = napi.lt(a as number, b as number, arg as NumType);
        stack.push(result);
        break;
      }
      case "ge": {
        const b = pop(), a = pop();
        if (arg === undefined) return 0;
        let result: boolean = instr.ge(a, b, ip, hitCounter, HOT_THRESHOLD, ctx, arg as NumType);
        stack.push(result);
        break;
      }
      case "le": {
        const b = pop(), a = pop();
        if (arg === undefined) return 0;
        let result: boolean = instr.le(a, b, ip, hitCounter, HOT_THRESHOLD, ctx, arg as NumType);
        stack.push(result);
        break;
      }
      case "eq": {
      ...
      case "typeof": {
        ...
        stack.push({
          class: normalizeType(getValueType(isStr ? val.replace("?", "") : val)).replace("$Type.", ""),
          nullable: isStr && val.endsWith("?") ? true : false
        });
        break;
      }
      case "instantiate": {
