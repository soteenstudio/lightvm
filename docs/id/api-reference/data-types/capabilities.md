...
`unsafeMode` menonaktifkan pemeriksaan keamanan seperti pembatasan impor dan kuota sumber daya untuk I/O, impor, alokasi, pemanggilan fungsi, dan lompatan, tetapi tidak memberikan izin kapabilitas. Verifikasi batas tetap aktif. Pemanggilan `halt` tetap memerlukan kapabilitas `Unsafe` dalam `VMConfig.caps`, meskipun `unsafeMode` diaktifkan.
