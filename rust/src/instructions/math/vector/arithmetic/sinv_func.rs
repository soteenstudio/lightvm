...
mod tests {
  ...
  fn requires_float_elements_and_accepts_cross_width_values() {
    assert!(sinv_values(array(vec![Value::Float64(0.0)]), PrimitiveTypes::Hlf, 24).is_ok());
    for invalid in [Value::Int32(1), Value::Bool(false)] {
      let value = array(vec![invalid.clone()]);
      assert!(matches!(
        sinv_values(value.clone(), PrimitiveTypes::Flt, 25),
      ...
      assert!(sinv_func(&mut stack, PrimitiveTypes::Flt, 26).is_err());
