...
mod tests {
  ...
  fn rejects_integer_elements_for_float_directives() {
    ...
    assert!(matches!(
      subv_values(left.clone(), right.clone(), PrimitiveTypes::Flt, 26),
    ...
    assert!(subv_func(&mut stack, PrimitiveTypes::Flt, 27).is_err());
    assert_eq!(stack, original);
  }
  #[test]
  fn rejects_float_elements_for_integer_directives() {
    let left = array(vec![Value::Int32(2)]);
    let right = array(vec![Value::Float64(1.0)]);
    assert!(matches!(
      subv_values(left.clone(), right.clone(), PrimitiveTypes::Int, 24),
      Err(VMError::TypeMismatch {
        ip: 24,
        expected: "Integer",
        found: "Double"
    ...
    let mut stack = Stack::from_vec(vec![left, right]);
    let original = stack.clone();
    assert!(subv_func(&mut stack, PrimitiveTypes::Int, 25).is_err());
    assert_eq!(stack, original);
  }
  #[test]
  ...
  fn underflow_preserves_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      subv_func(&mut stack, PrimitiveTypes::Int, 23),
