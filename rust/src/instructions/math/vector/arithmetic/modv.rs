...
use crate::instructions::math::arithmetic::mod_func::mod_values;
...
pub fn modv_values(a: Value, b: Value, num_type: PrimitiveTypes) -> Value {
  ...
  array(
    a.iter()
      .zip(b.iter())
      .map(|(a, b)| mod_values(a.clone(), b.clone(), num_type))
      .collect(),
  )
}

pub fn modv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  binary_func(stack, ip, "MODV", |a, b| modv_values(a, b, num_type))
}
