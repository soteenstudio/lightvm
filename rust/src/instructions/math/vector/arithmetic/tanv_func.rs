...
#[inline(always)]
pub fn tanv_values(a_val: Value, num_type: PrimitiveTypes, ip: usize) -> Result<Value, VMError> {
...
pub fn tanv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  let value = stack
    .last()
    .cloned()
    .ok_or(VMError::StackUnderflow { ip, opcode: "TANV" })?;
  let result = tanv_values(value, num_type, ip)?;
...
mod tests {
  ...
  fn requires_float_elements_and_accepts_cross_width_values() {
    assert!(tanv_values(array(vec![Value::Float64(0.0)]), PrimitiveTypes::Flt, 24).is_ok());
    for invalid in [Value::Int16(1), Value::Bool(false)] {
      let value = array(vec![invalid.clone()]);
      assert!(matches!(
        tanv_values(value.clone(), PrimitiveTypes::Flt, 25),
      ...
      assert!(tanv_func(&mut stack, PrimitiveTypes::Flt, 26).is_err());
  ...
  fn reports_type_mismatch_without_mutating_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      tanv_func(&mut stack, PrimitiveTypes::Flt, 17),
  ...
  fn validates_elements_and_directives() {
    assert!(tanv_values(array(vec![Value::Float32(1.0)]), PrimitiveTypes::Flt, 11).is_ok());
    assert!(matches!(
      tanv_values(array(vec![Value::Bool(false)]), PrimitiveTypes::Flt, 19),
    ...
    assert!(matches!(
      tanv_values(array(vec![Value::Float32(1.0)]), PrimitiveTypes::Str, 20),
  ...
  fn underflow_preserves_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      tanv_func(&mut stack, PrimitiveTypes::Flt, 23),
