...
use crate::instructions::math::arithmetic::add_func::add_values;
...
pub fn addv_values(a: Value, b: Value, num_type: PrimitiveTypes) -> Value {
  ...
  array(
    a.iter()
      .zip(b.iter())
      .map(|(a, b)| add_values(a.clone(), b.clone(), num_type))
      .collect(),
  )
}

pub fn addv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  binary_func(stack, ip, "ADDV", |a, b| addv_values(a, b, num_type))
}
