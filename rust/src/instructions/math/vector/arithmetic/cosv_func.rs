...
#[inline(always)]
pub fn cosv_values(a_val: Value, num_type: PrimitiveTypes, ip: usize) -> Result<Value, VMError> {
...
pub fn cosv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  let value = stack
    .last()
    .cloned()
    .ok_or(VMError::StackUnderflow { ip, opcode: "COSV" })?;
  let result = cosv_values(value, num_type, ip)?;
...
mod tests {
  ...
  fn reports_type_mismatch_without_mutating_stack() {
    ...
    assert!(matches!(
      cosv_func(&mut stack, PrimitiveTypes::Flt, 17),
  ...
  fn validates_elements_and_directives() {
    assert!(cosv_values(array(vec![Value::Float32(1.0)]), PrimitiveTypes::Flt, 11).is_ok());
    assert!(matches!(
      cosv_values(array(vec![Value::Bool(false)]), PrimitiveTypes::Flt, 19),
    ...
    assert!(matches!(
      cosv_values(array(vec![Value::Float32(1.0)]), PrimitiveTypes::Str, 20),
  ...
  fn underflow_preserves_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      cosv_func(&mut stack, PrimitiveTypes::Flt, 23),
