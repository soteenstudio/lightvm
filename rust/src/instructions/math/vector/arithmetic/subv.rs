...
use crate::instructions::math::arithmetic::sub_func::sub_values;
...
pub fn subv_values(a: Value, b: Value, num_type: PrimitiveTypes) -> Value {
  ...
  array(
    a.iter()
      .zip(b.iter())
      .map(|(a, b)| sub_values(a.clone(), b.clone(), num_type))
      .collect(),
  )
}

pub fn subv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  binary_func(stack, ip, "SUBV", |a, b| subv_values(a, b, num_type))
}
