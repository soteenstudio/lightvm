...
mod tests {
  ...
  fn supports_float_directives() {
    for num_type in [
      ...
      assert!(sqrtv_values(array(vec![Value::Float32(1.0)]), num_type, 0).is_ok());
    }
  }
  #[test]
  fn rejects_integer_elements_without_mutating_stack() {
    for (value, found) in [(Value::Int32(1), "Integer"), (Value::Int64(1), "Long")] {
      ...
      assert!(matches!(
        sqrtv_values(operand.clone(), PrimitiveTypes::Flt, 21),
      ...
      assert!(matches!(
        sqrtv_func(&mut stack, PrimitiveTypes::Flt, 22),
