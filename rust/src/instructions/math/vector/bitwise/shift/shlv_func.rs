...
use crate::instructions::math::vector::bitwise::shift::shlv::{
  shlv_i16in::shlv_i16in, shlv_i32in::shlv_i32in, shlv_i64in::shlv_i64in, shlv_i128in::shlv_i128in,
};
...
pub fn shlv_values(
  ...
  Ok(match num_type {
    PrimitiveTypes::Sht => Value::Array(shlv_i16in(&left, &right)),
    PrimitiveTypes::Int => Value::Array(shlv_i32in(&left, &right)),
    PrimitiveTypes::Lng => Value::Array(shlv_i64in(&left, &right)),
    PrimitiveTypes::Oct => Value::Array(shlv_i128in(&left, &right)),
    _ => {
      return Err(VMError::TypeMismatch {
        ip,
        expected: expected_type(num_type, ExpectedCategory::Integer),
...
#[inline]
pub fn shlv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  if stack.len() < 2 {
    return Err(VMError::StackUnderflow { ip, opcode: "SHLV" });
  }
  let result = shlv_values(
...
mod tests {
  ...
  fn validates_operands_and_preserves_stack() {
    ...
    for (left, right, num_type) in cases {
      ...
      assert!(shlv_func(&mut stack, num_type, 17).is_err());
  ...
  fn rejects_float_elements_without_mutating_stack() {
    for (left, right, found) in [
      ...
      assert!(matches!(
        shlv_values(left.clone(), right.clone(), PrimitiveTypes::Int, 20),
      ...
      let original = stack.clone();
      assert!(matches!(
        shlv_func(&mut stack, PrimitiveTypes::Int, 21),
  ...
  fn underflow_preserves_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      shlv_func(&mut stack, PrimitiveTypes::Int, 18),
