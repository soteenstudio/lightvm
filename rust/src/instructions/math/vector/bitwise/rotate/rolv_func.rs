...
use crate::instructions::math::vector::bitwise::rotate::rolv::{
...
mod tests {
  ...
  fn rejects_float_elements_without_mutating_stack() {
    for (left, right, found) in [
      ...
      assert!(matches!(
        rolv_values(left.clone(), right.clone(), PrimitiveTypes::Int, 20),
      ...
      let original = stack.clone();
      assert!(matches!(
        rolv_func(&mut stack, PrimitiveTypes::Int, 21),
