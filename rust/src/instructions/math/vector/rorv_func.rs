...
pub fn rorv_values(
  ...
  Ok(match num_type {
    PrimitiveTypes::Sht => Value::Array(rorv_i16in(&left, &right)),
    PrimitiveTypes::Int => Value::Array(rorv_i32in(&left, &right)),
    PrimitiveTypes::Lng => Value::Array(rorv_i64in(&left, &right)),
    PrimitiveTypes::Oct => Value::Array(rorv_i128in(&left, &right)),
    _ => {
      return Err(VMError::TypeMismatch {
        ip,
        expected: expected_type(num_type, ExpectedCategory::Integer),
...
pub fn rorv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  if stack.len() < 2 {
    return Err(VMError::StackUnderflow { ip, opcode: "RORV" });
  }
  let result = rorv_values(
...
mod tests {
  ...
  fn underflow_preserves_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      rorv_func(&mut stack, PrimitiveTypes::Int, 18),
