...
pub fn asinv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  ...
  let result = asinv_values(value, num_type, ip)?;
...
mod tests {
  ...
  fn reports_type_mismatch_without_mutating_stack() {
    ...
    assert!(matches!(
      asinv_func(&mut stack, PrimitiveTypes::Flt, 17),
  ...
  fn validates_elements_and_directives() {
    assert!(asinv_values(array(vec![Value::Float32(1.0)]), PrimitiveTypes::Flt, 11).is_ok());
    assert!(matches!(
      asinv_values(array(vec![Value::Bool(false)]), PrimitiveTypes::Flt, 19),
    ...
    assert!(matches!(
      asinv_values(array(vec![Value::Float32(1.0)]), PrimitiveTypes::Str, 20),
  ...
  fn underflow_preserves_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      asinv_func(&mut stack, PrimitiveTypes::Flt, 23),
