...
pub fn atanv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  ...
  let result = atanv_values(value, num_type, ip)?;
...
mod tests {
  ...
  fn reports_type_mismatch_without_mutating_stack() {
    ...
    assert!(matches!(
      atanv_func(&mut stack, PrimitiveTypes::Flt, 17),
  ...
  fn validates_elements_and_directives() {
    ...
    assert!(matches!(
      atanv_values(array(vec![Value::Bool(false)]), PrimitiveTypes::Flt, 19),
    ...
    assert!(matches!(
      atanv_values(array(vec![Value::Float32(1.0)]), PrimitiveTypes::Str, 20),
  ...
  fn underflow_preserves_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      atanv_func(&mut stack, PrimitiveTypes::Flt, 23),
