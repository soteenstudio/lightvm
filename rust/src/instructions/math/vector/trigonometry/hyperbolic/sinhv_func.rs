...
pub fn sinhv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  ...
  let result = sinhv_values(value, num_type).map_err(|found| VMError::TypeMismatch {
    ip,
...
mod tests {
  ...
  #[test]
  fn sinhv_dbl_works_and_preserves_preceding_stack_values() {
    let expected = array(vec![Value::Float64(0.0)]);
    assert_eq!(
      sinhv_values(array(vec![Value::Float64(0.0)]), PrimitiveTypes::Dbl),
      Ok(expected.clone())
    );
    let mut stack = Stack::from_vec(vec![Value::Bool(true), array(vec![Value::Float64(0.0)])]);
    sinhv_func(&mut stack, PrimitiveTypes::Dbl, 12).unwrap();
    assert_eq!(stack, Stack::from_vec(vec![Value::Bool(true), expected]));
  }
  #[test]
  fn sinhv_rejects_invalid_elements_without_mutating_stack() {
    ...
    assert!(matches!(
      sinhv_func(&mut stack, PrimitiveTypes::Dbl, 13),
  ...
  #[test]
  fn sinhv_handles_non_array_input_and_underflow() {
    let mut stack = Stack::from_vec(vec![Value::Bool(false)]);
    sinhv_func(&mut stack, PrimitiveTypes::Dbl, 14).unwrap();
    assert_eq!(stack, Stack::from_vec(vec![Value::NaN]));
    assert!(matches!(
      sinhv_func(&mut Stack::new(), PrimitiveTypes::Dbl, 15),
