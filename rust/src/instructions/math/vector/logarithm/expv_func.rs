...
#[inline(always)]
pub fn expv_values(value: Value, num_type: PrimitiveTypes, ip: usize) -> Result<Value, VMError> {
  ...
  Ok(match num_type {
    PrimitiveTypes::Hlf => Value::Array(expv_f16in(&values)),
    PrimitiveTypes::Flt => Value::Array(expv_f32in(&values)),
    PrimitiveTypes::Dbl => Value::Array(expv_f64in(&values)),
    _ => {
      return Err(VMError::TypeMismatch {
        ip,
        expected: expected_type(num_type, ExpectedCategory::Float),
...
#[inline]
pub fn expv_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  let value = stack
    .last()
    .cloned()
    .ok_or(VMError::StackUnderflow { ip, opcode: "EXPV" })?;
  let result = expv_values(value, num_type, ip)?;
...
mod tests {
  ...
  fn underflow_preserves_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      expv_func(&mut stack, PrimitiveTypes::Flt, 20),
