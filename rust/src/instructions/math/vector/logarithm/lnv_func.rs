...
mod tests {
  ...
  fn supports_float_directives() {
    assert!(matches!(
      lnv_values(array(vec![Value::Float64(1.0)]), PrimitiveTypes::Hlf, 0),
      Ok(Value::Array(values)) if matches!(&values[0], Value::Float16(_))
    ));
    assert!(matches!(
      lnv_values(array(vec![Value::Float16(half::f16::ONE)]), PrimitiveTypes::Dbl, 1),
  ...
  fn validates_operands_and_preserves_stack() {
    for (value, found) in [
      ...
      assert!(matches!(
        lnv_values(value.clone(), PrimitiveTypes::Flt, 16),
    ...
    assert!(matches!(
      lnv_values(value.clone(), PrimitiveTypes::Int, 18),
      Err(VMError::TypeMismatch { ip: 18, expected: "Float", found: "unknown" })
    ));
    let mut stack = Stack::from_vec(vec![value]);
    let original = stack.clone();
    assert!(lnv_func(&mut stack, PrimitiveTypes::Int, 18).is_err());
    assert_eq!(stack, original);
  }
  #[test]
