...
use crate::instructions::math::vector::logarithm::log10v::{
  log10v_f16in::log10v_f16in, log10v_f32in::log10v_f32in, log10v_f64in::log10v_f64in,
};
...
pub fn log10v_values(value: Value, num_type: PrimitiveTypes, ip: usize) -> Result<Value, VMError> {
  ...
  Ok(match num_type {
    ...
    PrimitiveTypes::Dbl => Value::Array(log10v_f64in(&values)),
    _ => {
      return Err(VMError::TypeMismatch {
        ip,
        expected: expected_type(num_type, ExpectedCategory::Float),
...
pub fn log10v_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  ...
  let result = log10v_values(value, num_type, ip)?;
...
mod tests {
  ...
  fn underflow_preserves_stack() {
    ...
    let original = stack.clone();
    assert!(matches!(
      log10v_func(&mut stack, PrimitiveTypes::Flt, 20),
