...
mod tests {
  ...
  fn invalid_operand_reports_type_mismatch_and_preserves_stack() {
    ...
    assert!(matches!(
      tan_values(Value::Int32(1), PrimitiveTypes::Flt, 17),
      Err(VMError::TypeMismatch {
        ip: 17,
        expected: "Float",
        found: "int32"
  ...
  fn accepts_cross_width_float_operands_and_returns_directive_type() {
    assert!(matches!(
      tan_values(Value::Float16(half::f16::ZERO), PrimitiveTypes::Flt, 19),
      Ok(Value::Float32(0.0))
    ));
    assert!(matches!(
      tan_values(Value::Float64(0.0), PrimitiveTypes::Hlf, 20),
      Ok(Value::Float16(value)) if value == half::f16::ZERO
    ));
    assert!(matches!(
      tan_values(Value::Float64(0.0), PrimitiveTypes::Flt, 21),
      Ok(Value::Float32(0.0))
    ));
  }
}
