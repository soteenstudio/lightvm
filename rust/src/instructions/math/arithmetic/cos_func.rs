...
use crate::instructions::math::arithmetic::cos::{
...
pub fn cos_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  ...
  let result = cos_values(val, num_type, ip)?;
...
mod tests {
  ...
  fn invalid_operand_reports_type_mismatch_and_preserves_stack() {
    ...
    assert!(matches!(
      cos_values(invalid.clone(), PrimitiveTypes::Flt, 17),
    ...
    assert!(matches!(
      cos_func(&mut stack, PrimitiveTypes::Flt, 18),
      Err(VMError::TypeMismatch {
        ip: 18,
        expected: "Float",
        found: "string"
