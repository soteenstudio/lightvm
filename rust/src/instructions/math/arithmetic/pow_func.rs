...
#[inline]
pub fn pow_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  if stack.len() < 2 {
    return Err(VMError::StackUnderflow { ip, opcode: "POW" });
  }
  let result = pow_values(
...
mod tests {
  ...
  fn invalid_operands_report_type_mismatch_and_preserve_stack() {
    ...
    assert!(matches!(
      pow_values(invalid.clone(), Value::Int32(1), PrimitiveTypes::Int, 17),
    ...
    assert!(matches!(
      pow_values(Value::Int32(1), invalid.clone(), PrimitiveTypes::Int, 18),
      Err(VMError::TypeMismatch {
        ip: 18,
        expected: "Integer",
        found: "string"
    ...
    assert!(matches!(
      pow_func(&mut stack, PrimitiveTypes::Int, 19),
      Err(VMError::TypeMismatch {
        ip: 19,
        expected: "Integer",
        found: "string"
