...
#[inline]
pub fn shr_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  if stack.len() < 2 {
    return Err(VMError::StackUnderflow { ip, opcode: "SHR" });
  }
  let result = shr_values(
...
mod tests {
  use super::*;
  #[test]
  fn rejects_float_operands_without_mutating_stack() {
    for (a, b, found) in [
      (Value::Float32(1.0), Value::Int32(1), "float32"),
      (Value::Int32(1), Value::Float64(1.0), "float64"),
    ] {
      assert!(matches!(
        shr_values(a.clone(), b.clone(), PrimitiveTypes::Int, 20),
      ...
      assert!(matches!(
        shr_func(&mut stack, PrimitiveTypes::Int, 21),
  ...
  fn invalid_operands_report_type_mismatch_and_preserve_stack() {
    ...
    assert!(matches!(
      shr_values(invalid.clone(), Value::Int32(1), PrimitiveTypes::Int, 17),
    ...
    assert!(matches!(
      shr_values(Value::Int32(1), invalid.clone(), PrimitiveTypes::Int, 18),
      Err(VMError::TypeMismatch {
        ip: 18,
        expected: "Integer",
        found: "string"
    ...
    let original = stack.clone();
    assert!(matches!(
      shr_func(&mut stack, PrimitiveTypes::Int, 19),
      Err(VMError::TypeMismatch {
        ip: 19,
        expected: "Integer",
        found: "string"
