...
pub fn atanh_values(a: Value, num_type: PrimitiveTypes, ip: usize) -> Result<Value, VMError> {
  ...
  Ok(match num_type {
    PrimitiveTypes::Hlf => Value::Float16(atanh_f16in(a.as_f16())),
    PrimitiveTypes::Flt => Value::Float32(atanh_f32in(a.as_f32())),
    PrimitiveTypes::Dbl => Value::Float64(atanh_f64in(a.as_f64())),
    _ => {
      return Err(VMError::TypeMismatch {
        ip,
        expected: expected_type(num_type, ExpectedCategory::Float),
...
#[inline]
pub fn atanh_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  let val = stack.last().cloned().ok_or(VMError::StackUnderflow {
    ip,
    opcode: "ATANH",
  })?;
  let result = atanh_values(val, num_type, ip)?;
