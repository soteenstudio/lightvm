...
pub fn acosh_values(a: Value, num_type: PrimitiveTypes, ip: usize) -> Result<Value, VMError> {
  ...
  Ok(match num_type {
    PrimitiveTypes::Hlf => Value::Float16(acosh_f16in(a.as_f16())),
    PrimitiveTypes::Flt => Value::Float32(acosh_f32in(a.as_f32())),
    PrimitiveTypes::Dbl => Value::Float64(acosh_f64in(a.as_f64())),
    _ => {
      return Err(VMError::TypeMismatch {
        ip,
        expected: expected_type(num_type, ExpectedCategory::Float),
...
mod tests {
  ...
  fn reports_errors_without_mutating_stack() {
    crate::instructions::math::assert_unary_float_errors(super::acosh_func, "ACOSH");
  }
}
