...
#[inline(always)]
pub fn acos_values(a: Value, num_type: PrimitiveTypes, ip: usize) -> Result<Value, VMError> {
  if !a.is_number() {
...
pub fn acos_func(stack: &mut Stack, num_type: PrimitiveTypes, ip: usize) -> Result<(), VMError> {
  ...
  let result = acos_values(val, num_type, ip)?;
...
mod tests {
  ...
  fn reports_errors_without_mutating_stack() {
    crate::instructions::math::assert_unary_float_errors(super::acos_func, "ACOS");
  }
}
