  ...
  match inst {
    Instructions::InitStack(size) => {
      ...
      builder
        ...
        .sub("sp", "sp", &format!("#{}", bytes))
    }
    Instructions::PushInt16(val) => builder
      .comment(&format!("PushInt16({})", val))
      .inst("movz", &format!("x9, #{}", val))
      .sub("sp", "sp", "#16")
      .str("x9", "sp"),
    Instructions::PushInt32(val) => builder
    ...
    Instructions::PushInt128(val) => {
      let v = *val as u128;
      let low = (v & 0xFFFF) as u64;
      let mid1 = ((v >> 16) & 0xFFFF) as u64;
      let mid2 = ((v >> 32) & 0xFFFF) as u64;
      let high = ((v >> 48) & 0xFFFF) as u64;
      builder = builder
    ...
    Instructions::PushArray(arr) => builder
      .comment(&format!("PushArray(len={})", arr.len()))
      .inst("mov", "x9, #0")
      .sub("sp", "sp", "#16")
      .str("x9", "sp"),
    Instructions::PushObject(obj) => builder
      .comment(&format!("PushObject(len={})", obj.len()))
      .inst("mov", "x9, #0")
      .sub("sp", "sp", "#16")
      .str("x9", "sp"),
    ...
    Instructions::GetIdx(idx) => {
      ...
      builder
        ...
        .inst("ldr", &format!("x9, [sp, #{}]", offset))
        .sub("sp", "sp", "#16")
        .str("x9", "sp")
    }
    Instructions::Concat => builder.comment("Concat").inst("mov", "x9, #0"),
    ...
    Instructions::Swap => builder
      ...
      .inst("str", "x9, [sp, #16]")
      .inst("str", "x10, [sp]"),
    Instructions::Truncate => builder.comment("Truncate"),
    _ => builder,
  }
}
